...
/*-----------------------------------------------------------------------*/
/* Deselect_1 card and release SPI                                         */
/*-----------------------------------------------------------------------*/

static void deselect_1 (void)
{
	FATFS_CS_HIGH;			/* CS = H */
	TM_SPI_Send(FATFS_SPI, 0xFF);			/* Dummy clock (force DO hi-z for multiple slave SPI) */
	FATFS_DEBUG_SEND_USART("deselect_1: ok");
}



/*-----------------------------------------------------------------------*/
/* select_1 card and wait for ready                                        */
/*-----------------------------------------------------------------------*/

static int select_1 (void)	/* 1:OK, 0:Timeout */
{
	FATFS_CS_LOW;
	TM_SPI_Send(FATFS_SPI, 0xFF);	/* Dummy clock (force DO enabled) */

	if (wait_ready(500)) {
		FATFS_DEBUG_SEND_USART("select_1: OK");
		return 1;	/* OK */
	}
	FATFS_DEBUG_SEND_USART("select_1: no");
	deselect_1();
	return 0;	/* Timeout */
}
...
)
{
	...
	/* select_1 the card and wait for ready except to stop multiple block read */
	if (cmd != CMD12) {
		deselect_1();
		if (!select_1()) return 0xFF;
	}
